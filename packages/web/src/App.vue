<template>
  <div class="w-full bg-bgGrayLight pb-32 min-h-screen">
    <div class="relative w-full h-906 -mb-240 bg-no-repeat bg-cover bg-[url('./assets/headerBg.jpg')]">
      <div class="absolute w-600 top-200 en-GB:top-300 left-32 text-72">这是一个slogan，slogan</div>
    ...
    <div class="relative mx-32 mt-32 px-32 py-64 bg-white rounded-12 text-24">
      ...
      <select v-model="gsmInfo.gsmCode" placeholder="请选择地域" class="w-300 h-60 text-32 mb-12 outline-none">
        <option v-for="item in gsmInfo.gsmOptions" :key="item.value" :label="item.label" :value="item.value"></option>
      </select>
